...
metrics_file = "training_metrics_PRI.csv"
    ...
    def replay(self):
        ...
        for state, action, reward, next_state, done, _ in minibatch:
            ...
            if done:
                ...
                t = self.target_q_network.predict(state)
                target[0][action] = reward + self.gamma * np.amax(t[0])
            self.q_network.update(state, target)
...
def train_agent(state_size, action_size, memory_size, batch_size, episodes, max_steps):
    ...
    for e in range(1, episodes+1):
        ...
        if e % 100 == 0:
            agent.q_network.model.save(f"ddqn_model_PRI_{e}.h5")
    return agent
...
if __name__ == '__main__':
    ...
    trained_agent.q_network.model.save("trained_ddqn_model_PRI.h5")
