...
#incorporated restart algorithm per episode if it reaches 200+ moves


metrics_file = "training_metrics_PRI6.csv"
...
def train_agent_with_selfplay_restart(agent, game, max_restart, max_steps, e,  L = 100):
    tstart = 0
    tend = float('inf')
    restarts_done = 0
...
def train_agent(agent, episodes, max_steps):
    ...
    for e in range(1, episodes+1):
        ...
        train_agent_with_selfplay_restart(agent, game, 3, max_steps, e)
        if e % 100 == 0:
            agent.q_network.model.save(f"ddqn_model_PRI6_{e}.h5")
...
if __name__ == '__main__':
    ...
    agent.q_network.model.save("trained_ddqn_model_PRI6.h5")
