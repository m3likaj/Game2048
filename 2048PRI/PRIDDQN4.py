...
#incorporated restart algorithm


metrics_file = "training_metrics_PRI4.csv"
if not os.path.exists(metrics_file):
    with open(metrics_file, mode="w", newline="") as file:
        writer = csv.writer(file)
        # Write header row
        writer.writerow(["Restart", "Score", "Invalid Moves", "Highest Tile", "Total Reward", "Epsilon"])
...
def train_agent_with_selfplay_restart(state_size, action_size, memory_size, batch_size, episodes, max_steps):
    tstart = 0
    tend = float('inf')
    restarts_done = 0
    L = 10  # length threshold
    agent = Agent(state_size, action_size, memory_size, batch_size)
    game = Game.Game2048()

    print("Agent created")

    while (tend - tstart) > L and restarts_done < episodes:
        restarts_done += 1
        total_reward = 0
        transitions = []
        if restarts_done == 1:
        ...
        for step in range(max_steps):
            ...
            reward = game.get_reward(prev_board)  # or pass old_board, etc.
        ...
        print(
            ...
            f"Highest={highest_tile}, Invalid={invalid_moves}, Reward={total_reward}"
        )
        log_metrics(restarts_done, score, invalid_moves, highest_tile, total_reward, agent.epsilon)
        if restarts_done % 100 == 0:
            agent.q_network.model.save(f"ddqn_model_PRI4_{e}.h5")
...
if __name__ == '__main__':
    ...
    trained_agent = train_agent_with_selfplay_restart(state_size, action_size, memory_size, batch_size, episodes, max_steps)
    trained_agent.q_network.model.save("trained_ddqn_model_PRI4.h5")
