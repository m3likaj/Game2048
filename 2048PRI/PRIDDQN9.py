...
# attempting to use expectimax



metrics_file = "training_metrics_PRI9.csv" # replaced reward with heuristic
...
def train_agent_with_selfplay_restart(agent, game, max_restart,pool, max_steps, e,  L = 50):
    tstart = 0
    tend = float('inf')
    restarts_done = 0
    ...
    while (tend - tstart) > L and restarts_done < max_restart:
        ...
        for step in range(max_steps):
            ...
            if e % 10 == 0:
                dir = getNextBestMoveExpectiminimax(game, pool, 2)
                if dir == 'left':
                    action = 0
                elif dir == 'right':
                    action = 1
...
def train_agent(agent, episodes, max_steps):
    ...
    for e in range(1, episodes+1):
        ...
        train_agent_with_selfplay_restart(agent, game, 10,pool, max_steps, e)
        if e % 200 == 0:
            agent.q_network.model.save(f"ddqn_model_PRI9_{e}.h5")
    pool.close()
    pool.terminate()
...
if __name__ == '__main__':
    ...
    agent.q_network.model.save("trained_ddqn_model_PRI9.h5")
