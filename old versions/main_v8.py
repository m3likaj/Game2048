...
# this is V5 with expectimax fined for invalid moves and rewards adjusted
PERFECT_SNAKE = [[2, 2 ** 2, 2 ** 3, 2 ** 4],
                     [2 ** 8, 2 ** 7, 2 ** 6, 2 ** 5],
                     [2 ** 9, 2 ** 10, 2 ** 11, 2 ** 12],
                     [2 ** 16, 2 ** 15, 2 ** 14, 2 ** 13]]

metrics_file = "training_metrics_v6.csv"
...
def depth_limited_expectimax_search(
    ...
    if is_agent:
        ...
        for (r, c) in empty_positions:
            ...
            if (value_sum - current_best) < -threshold:
                break
...
class DDQNAgent:
    ...
    def act_with_expectimax(self, game, max_depth=2, threshold=200):
        """
    ...
    def act(self, state, game=None):
        ...
        if self.use_expectimax and game is not None:
            return self.act_with_expectimax(game, max_depth=2, threshold=200)
...
def train_ddqn(agent, episodes=2000, update_target_every=10):
    ...
    for episode in range(1, episodes + 1):
        ...
        if episode % 200 == 0:
            agent.main_model.save(f"ddqn_model_v8_{episode}.h5")
...
if __name__ == '__main__':
    ...
    trained_agent.main_model.save("trained_ddqn_model_v8.h5")
