...
# this is V6 with fixed rewards

metrics_file = "training_metrics_v7.csv"
...
def heuristic_2048(board):
    """
    Combines several common heuristics:
      1. Monotonicity (reward sequences that are strictly increasing or decreasing)
      2. Smoothness (penalize large differences between adjacent cells)
      3. Empty cells (reward having free space)
      4. Max tile (slight reward for the largest tile on the board)

    Returns a single scalar value. Higher means "better" board state.
    """
    # Convert to float to avoid integer division issues
    board = board.astype(float)

    # 1. Monotonicity
    # We'll measure how "ordered" each row and column is.
    # The more monotonic each line is, the better.
    # We'll sum up partial monotonicity from rows and columns.
    def monotonicity_score(line):
        """
        Returns a measure of how strictly increasing or decreasing 'line' is.
        Higher is more monotonic.
        """
        inc, dec = 0.0, 0.0
        for i in range(len(line) - 1):
            if line[i] > line[i + 1]:
                ...
                inc += line[i + 1] - line[i]
        # We take the minimum because a line can't be both strictly inc and dec.
        # The line’s monotonicity is stronger if it's mostly one direction.
        return -min(inc, dec)
    ...
    for r in range(4):
        ...
        if row:
            mono_score += monotonicity_score(row)
    # Columns
    for c in range(4):
        col = [board[r, c] for r in range(4) if board[r, c] != 0]
        if col:
            mono_score += monotonicity_score(col)

    # 2. Smoothness
    # Penalize large differences between neighboring tiles.
    # We'll look at both horizontal and vertical adjacents.
    ...
    # 3. Empty Cells
    # Reward the number of zeroes in the board
    empty_cells = np.count_nonzero(board == 0)

    # 4. Max Tile
    # Slightly reward having a large tile
    max_tile = np.max(board)

    # Combine them into a single value:
    # Tune these weights as needed
    alpha = 1.0  # weight for monotonicity
    beta = 1.0  # weight for smoothness (note it’s negative)
    gamma = 3  # weight for empty cells
    delta = 1.5  # weight for max tile

    # Notice we add smoothness_score (which is negative),
    # so we might do: alpha * mono_score + beta * smoothness_score ...
    # or invert it, depending on your preference.
    # Typically, smoothness is negative, so you add it directly:
    score = (alpha * mono_score
             + beta * smoothness_score
             + gamma * empty_cells
             + delta * np.log2(max_tile))
...
def depth_limited_expectimax_search(
    ...
    if depth == max_depth or not game.can_move():
        ...
        return heuristic_2048(game.get_board())
...
class DDQNAgent:
    ...
    def act_with_expectimax(self, game, max_depth=4, threshold=300):
        """
...
def train_ddqn(agent, episodes=2000, update_target_every=10):
    game = Game2048()
    ...
    for episode in range(1, episodes + 1):
        ...
        if episode % 100 == 0:
            agent.main_model.save(f"ddqn_model_v7_{episode}.h5")
...
if __name__ == '__main__':
    ...
    trained_agent.main_model.save("trained_ddqn_model_v7.h5")
